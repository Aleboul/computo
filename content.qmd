...
The $\textsf{clayton}$ package, whose Python code can be found in this [GitHub repository](https://github.com/Aleboul/clayton), uses object-oriented features of the Python language. The package contains classes for Archimedean, elliptical, and extreme value copulae. In section @sec-classes, we briefly describe the classes defined in the package. @sec-rng presents methods for generating random vectors. In section @sec-pairwise, we apply the $\textsf{clayton}$ package to model pairwise dependence between maxima. @sec-discussion discusses potential improvements to the package and provides concluding remarks. The from @sec-bv_arch to @sec-mv_ellip define and illustrate all the parametric copula models implemented in the package.
